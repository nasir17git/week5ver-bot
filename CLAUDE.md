...
- `/목표등록`, `/목표조회`, `/목표인증`, `/등록발송`, `/인증발송`, `/알림발송` 슬래시 명령어 및 이모지를 통한 목표 등록/갱신 가능
...
- [x] 매일 21:00 KST 미완료 항목 담당자 DM 알림 (`send_daily_notifications`) + `/일간알림발송` 수동 트리거
...
- **슬래시 명령어 등록**: `/목표등록`, `/목표조회`, `/목표인증`, `/등록발송`, `/인증발송`, `/알림발송`
