...
These are developer-focused instructions that cover posting messages to Slack via APIs.
...
[App surfaces](/surfaces) such as [messages](/messaging), [modals](/surfaces/modals), or [Home tabs](/surfaces/app-home) can contain all types of textual treasures. The text itself can be formatted and placed inside blocks to create [rich message layouts](#rich-layouts).

The [Block Kit](/block-kit) elements that form the layouts for these surfaces frequently use [text objects](/reference/block-kit/composition-objects/text-object) to insert all kinds of content. These text objects can be left as unformatted `plain_text` or formatted with `mrkdwn` to create **useful visual highlights** or to utilize a certain **syntax to trigger special parsing**. This reference guide explains all of the `mrkdwn` formatting options available to use in text objects.

These instructions are also handy if you are [retrieving messages](/messaging/retrieving-messages) and want to understand how to interpret the formatting syntax.
...
## Basic formatting with mrkdwn {#basic-formatting}

Text objects contain a `text` field that can be formatted using a markup language called `mrkdwn`.

Use it in [most Block Kit text objects](/reference/block-kit/composition-objects/text-object) by specifying a `type` of `mrkdwn`. There are, however, a few Block Kit blocks and elements that only allow `plain_text` with no formatting — these are called out in the [Block Kit reference guides](/reference/block-kit/blocks).

`mrkdwn` is also the default formatting method for the top-level `text` field in a message object when using the [Web API to publish a message](/messaging/sending-and-scheduling-messages#getting_started).

`mrkdwn` can be used within [secondary message attachments](#attachments) by using the `mrkdwn_in` field [as shown in the relevant reference guide](/legacy/legacy-messaging/legacy-secondary-message-attachments). This formatting is similar to [Markdown markup](https://daringfireball.net/projects/markdown/) but with its own syntax, as shown below.

### Visual basics {#visual-styles}
...
[View in Block Kit Builder](https://api.slack.com/tools/block-kit-builder?blocks=%5B%0A%09%7B%0A%09%09%22type%22%3A%20%22section%22%2C%0A%09%09%22text%22%3A%20%7B%0A%09%09%09%22type%22%3A%20%22mrkdwn%22%2C%0A%09%09%09%22text%22%3A%20%22%E2%80%A2%20Detective%20Chimp%5Cn%E2%80%A2%20Bouncing%20Boy%5Cn%E2%80%A2%20Aqualad%22%0A%09%09%7D%0A%09%7D%0A%5D)
...
[View in Block Kit Builder](https://api.slack.com/tools/block-kit-builder?blocks=%5B%0A%09%7B%0A%09%09%22type%22%3A%20%22section%22%2C%0A%09%09%22text%22%3A%20%7B%0A%09%09%09%22type%22%3A%20%22mrkdwn%22%2C%0A%09%09%09%22text%22%3A%20%22This%20message%20contains%20a%20URL%20http%3A%2F%2Fexample.com%2F%5CnSo%20does%20this%20one%3A%20www.example.com%5CnThis%20message%20contains%20a%20URL%20%3Chttp%3A%2F%2Fexample.com%2F%3E%5Cn%3Chttp%3A%2F%2Fwww.example.com%7CThis%20message%20*is*%20a%20link%3E%5Cn%3Cmailto%3Abob%40example.com%7CEmail%20Bob%20Roberts%3E%22%0A%09%09%7D%0A%09%7D%0A%5D)
...
When you use any of the [messaging APIs](/messaging/) to [retrieve a message](/messaging/retrieving-messages), you'll see that auto-transformed URLs are shown in the `mrkdwn` format. So this:
...
`#C123ABC456` is the channel's ID. Your app can get this ID from an [interaction request payload](/messaging/creating-interactive-messages#request), from the [Event API payload](/reference/events/app_mention) sent when [one of the event types](/reference/events) occurs, or by looking it up directly with [the conversation-related methods of the Web API](/apis/web-api/using-the-conversations-api).
...
If you're [retrieving messages](/messaging/retrieving-messages), use the [`conversations.info`](/reference/methods/conversations.info) Web API method to look up by channel ID and to find out other relevant information (such as the actual name of the channel).
...
If the mention is included in an app-published [message](/messaging), the mentioned user will also be notified about the reference. This [Help Center article](https://slack.com/help/articles/205240127-Mention-a-member) describes what that notification process looks like.
...
Your app can get this user ID from an [interaction request payload](/messaging/creating-interactive-messages#request), from the [Event API payload](/reference/events/app_mention) sent when [one of the event types](/reference/events) occurs, or by looking them up via the [`users.list` Web API](/reference/methods/users.list) using another unique piece of information you have about them, such as their email address.
...
If you're [retrieving messages](/messaging/retrieving-messages), you can use the [`users.info`](/reference/methods/users.info) Web API method to lookup by user ID and find out other relevant information, such as their display name.

### Mentioning groups {#mentioning-groups}

As with [users](#mentioning-users), you can mention [user groups](https://slack.com/help/articles/212906697-Create-a-user-group), which will link to the group's profile.

If the mention is included in an app-published [message](/messaging), Slack will notify each user in the group about the mention.
...
Your app can get this group ID from the [Event API payload](/reference/events/app_mention) sent when [one of the `subteam` event types](/reference/events) occurs, or by looking them up via the [`usergroups.list` Web API](/reference/methods/usergroups.list). You can also manually retrieve a specific user group's ID from the URL shown when viewing its profile.
...
If you're [retrieving messages](/messaging/retrieving-messages), you can use [the `usergroups.list` Web API method](/reference/methods/usergroups.list) to lookup by user group ID and find out other relevant information.

### Special mentions {#special-mentions}

These special groups should be mentioned sparingly, as they tend to notify a large group of users.

We don't have any specific guidelines, but use your discretion when mentioning [individuals](#mentioning-users) or [more specific groups](#mentioning-groups) where possible.
...
If you're [retrieving messages](/messaging/retrieving-messages), you will receive the `<!date>` string back in its original format, and you can use the information above to parse it if necessary.

* * *

### Automatic parsing {#automatic-parsing}

[Above](#advanced), we explained how to include various types of special syntax in your app-published text. What we didn't mention is that you _can_ just include the same text as a user would post directly in Slack to achieve some of these things. For example, given the following text:
...
*   For text in [layout blocks](/messaging#complex_layouts), set a `verbatim` attribute of your [text objects](/reference/block-kit/composition-objects/text-object) to `false`. This is actually the default method of processing these text objects.
*   For the top-level `text` field, or text in [secondary message attachments](#attachments), include a `link_names` argument with value of `1` when publishing the message — see the [reference docs for the relevant publishing method](/messaging/sending-and-scheduling-messages#publishing) for more information about this.
...
Even with this functionality available, _we still recommend that you use the manual methods shown [above](#advanced)_. (This is also why we didn't mention it earlier.)

This is important because the names of conversations or user groups may change at any time. What was previously a functioning reference may no longer work. Meanwhile, an ID will always remain the same. The same holds true for [special mentions](#special-mentions) like `@here`.
...
We've already [deprecated this functionality for user mentions](/changelog/2017-09-the-one-about-usernames), so always parsing manually will keep you prepared in case automatic parsing is deprecated for other entities such as conversations or user groups in the future.
...
We can take message formatting a step further by making [rich message layouts](/reference/block-kit/blocks/rich-text-block) with Block Kit, which enables us to structure complex data in a readable and understandable way within messages.
...
Blocks are added to messages by adding a `blocks` array to [the message payload](/messaging#payloads), like this:
...
*   [`chat.postMessage`](/reference/methods/chat.postMessage) and [`chat.postEphemeral`](/reference/methods/chat.postEphemeral) methods in the [Web API](/apis/web-api/).
...
Read our [guide to sending messages](/messaging/sending-and-scheduling-messages) to get started with that process for your app. Once you've decided which sending method to use, consult that method's reference guide to find out where to include your JSON payload. When you call your selected method, your stack of blocks will transform into a beautiful new message, like this:
...
### Adding secondary attachments {#attachments}

This feature is a legacy part of messaging functionality for Slack apps.

We recommend you stick with Block Kit as above, but if you still want to use attachments, [read our caveats below.](#when-to-use-attachments)
...
![Message with blocks showing a review of a hotel with 1 stars given, and secondary content showing alternative hotel options](/assets/images/secondary_attachment_example-77a4e51db62ec1b24e400e8d19d6d189.png)
...
```
{  "channel": "C123ABC456",  "attachments": [      {          "fallback": "Plain-text summary of the attachment.",          "color": "#2eb886",          "pretext": "Optional text that appears above the attachment block",          "author_name": "Bobby Tables",          "author_link": "http://flickr.com/bobby/",          "author_icon": "http://flickr.com/icons/bobby.jpg",          "title": "Slack API Documentation",          "title_link": "https://docs.slack.dev/",          "text": "Optional text that appears within the attachment",          "fields": [              {                  "title": "Priority",                  "value": "High",                  "short": false              }          ],          "image_url": "http://my-website.com/path/to/image.jpg",          "thumb_url": "http://example.com/path/to/thumb.png",          "footer": "Slack API",          "footer_icon": "https://platform.slack-edge.com/img/default_application_icon.png",          "ts": 123456789      }  ]}
```

Within the `attachments` array, you can also include a second-level `blocks` array, [constructed using Block Kit blocks and elements](#adding-blocks). Here's the JSON payload from the [Block Kit example we created before](#adding-blocks), this time inside an attachment:

```
{  "channel": "C123ABC456",  "blocks": [    {      "type": "section",      "text": {        "type": "mrkdwn",        "text": "Danny Torrence left the following review for your property:"      }    },    {      "type": "section",      "block_id": "section567",      "text": {        "type": "mrkdwn",        "text": "<https://example.com|Overlook Hotel> \n :star: \n Doors had too many axe holes, guest in room 237 was far too rowdy, whole place felt stuck in the 1920s."      },      "accessory": {        "type": "image",        "image_url": "https://is5-ssl.mzstatic.com/image/thumb/Purple3/v4/d3/72/5c/d3725c8f-c642-5d69-1904-aa36e4297885/source/256x256bb.jpg",        "alt_text": "Haunted hotel image"      }    },    {      "type": "section",      "block_id": "section789",      "fields": [        {          "type": "mrkdwn",          "text": "*Average Rating*\n1.0"        }      ]    }  ],  "attachments": [    {      "blocks": [      {          "type": "section",          "text": {            "type": "mrkdwn",            "text": "*Alternative hotel options*"          }        },        {          "type": "section",          "text": {            "type": "mrkdwn",            "text": "<https://example.com|Bates Motel> :star::star:"          },          "accessory": {            "type": "button",            "text": {              "type": "plain_text",              "text": "View",              "emoji": true            },            "value": "view_alternate_1"          }        },        {          "type": "section",          "text": {            "type": "mrkdwn",            "text": "<https://example.com|The Great Northern Hotel> :star::star::star::star:"          },          "accessory": {            "type": "button",            "text": {              "type": "plain_text",              "text": "View",              "emoji": true            },            "value": "view_alternate_2"          }        }      ]    }  ]}
```

Secondary attachments have a few additional parameters that can be included within each object in the `attachments` array. Take a look at our [reference guide for secondary attachments](/legacy/legacy-messaging/legacy-secondary-message-attachments) to see the full list.

### When should I use secondary attachments? {#when-to-use-attachments}
...
In short, we recommend you use [Block Kit](/block-kit) for as much of your message composition as you can, and avoid using attachments if possible. Blocks have many more visual and interactive capabilities available.
...
✨ **To learn about all the blocks and options available**, head to the [reference guides for layout blocks](/reference/block-kit/blocks).

✨ **To learn how messages can be used for more than just communicating data and how interactivity can trigger more fine-grained workflows**, head to the [overview of messaging interactivity](/messaging/creating-interactive-messages).
